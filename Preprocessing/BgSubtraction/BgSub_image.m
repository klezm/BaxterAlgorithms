function [oI, oBg] = BgSub_image(aImData, aFrame, aImage, aBreakPoints, varargin)
    % Subtracts an image from an image sequence.
    %
    % Subtracts an image from images of the sequence.
    %
    % Inputs:
    ...
    if isempty(aBreakPoints)
        ...
        % oBg = double(imread(aImage))/(2.^8-1)*255;
        oBg = double(imread(aImage));
        % GetUint8Image(imread(aImage));
